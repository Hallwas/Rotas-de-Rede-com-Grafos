...
//Gera um arquivo .dot igual ao grafo completo, mas destaca em azul os nos do caminho e em vermelho as arestas que fazem parte dele.
void exportar_com_caminho(string nomeArquivo, vector<string> caminho) {
    ...
    for (auto& par : nodes) {
        ...
        for (int i = 0; i < (int)nodo.links.size(); i++) {
            ...
            //Verifica se essa aresta faz parte do caminho
